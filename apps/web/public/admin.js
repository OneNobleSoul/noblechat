...
function renderStatus(s) {
  ...
  const pill = $("#maint-pill");
  pill.textContent = s.maintenance ? "enabled" : "disabled";
  pill.className = "pill " + (s.maintenance ? "on" : "off");
...
async function loadUsers() {
  ...
  for (const u of users) {
    ...
    tr.innerHTML = `<td>${esc(u.handle)}</td><td>${u.queued || 0}</td><td class="muted">${fmtTime(u.updated_at)}</td>`;
    const td = document.createElement("td"); td.style.textAlign = "right";
    const ban = document.createElement("button"); ban.className = "btn warn"; ban.textContent = "Ban";
    ban.onclick = () => act("ban", u.handle);
    const del = document.createElement("button"); del.className = "btn danger"; del.textContent = "Delete";
    del.onclick = () => act("delete", u.handle);
    td.appendChild(ban); td.appendChild(del); tr.appendChild(td); tb.appendChild(tr);
  }
  const bt = $("#bans"); bt.innerHTML = "";
  $("#bans-empty").classList.toggle("hidden", bans.length > 0);
  for (const b of bans) {
    const tr = document.createElement("tr");
    tr.innerHTML = `<td>${esc(b.handle)}</td><td class="muted">${esc(b.reason || "")}</td>`;
    const td = document.createElement("td"); td.style.textAlign = "right";
    const un = document.createElement("button"); un.className = "btn ghost"; un.textContent = "Unban";
    un.onclick = () => act("unban", b.handle);
    td.appendChild(un); tr.appendChild(td); bt.appendChild(tr);
  }
}
...
async function act(kind, handle) {
  ...
  else if (kind === "delete") { if (!confirm("Delete " + handle + "? This removes their card and queued messages.")) return; await api("/api/admin/delete", "POST", { handle }); }
