...
// `label` is folded into the coturn username so relay use is attributable to an
// account instead of a single shared "noblechat" credential everyone gets (see
// pentest M-5). coturn takes the expiry before the first ":" and HMACs the whole
// string, so any label without a ":" works; handles never contain one.
export function turnCredentials(secret, ttlSec = 3600, now = Date.now(), label = "noblechat") {
  const expiry = Math.floor(now / 1000) + Math.max(60, ttlSec);
  const safeLabel = String(label || "noblechat").replace(/:/g, "");
...
export function turnIceServers(cfg, now = Date.now(), label = "noblechat") {
  if (!cfg.turnSecret || !cfg.turnUris || !cfg.turnUris.length) return [];
  const { username, credential } = turnCredentials(cfg.turnSecret, cfg.turnTtlSec, now, label);
